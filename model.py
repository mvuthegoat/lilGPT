...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0):
    """Compute frequencies in complex form (cis(x) = cos(x) + i*sin(x) = e^ix)"""

    # Faster way to calculate the original position embedding function
    freqs = 1.0 / (theta ** (torch.arange(0, dim, 2)[: (dim // 2)].float() / dim))
    t = torch.arange(end, device=freqs.device)  # end is same as pos
...
def apply_rotary_emb(
    ...
    # flatten the tensor in the last 2 dimensions (the real and imaginary parts)
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # make cache's dtype and device same as xq's
        self.cache_k = self.cache_k.to(xq)
        self.cache_v = self.cache_v.to(xq)

        self.cache_k.detach_()
        self.cache_v.detach_()

        # update caches with new key/value. xk and xv only represent a single token
        self.cache_k[:bsz, start_pos : start_pos + seqlen] = xk
        self.cache_v[:bsz, start_pos : start_pos + seqlen] = xv

        # get the full keys and values correspond to the entire prompt token
        keys = self.cache_k[:bsz, : start_pos + seqlen]
        values = self.cache_v[:bsz, : start_pos + seqlen]

        # repeat k/v heads if n_kv_heads < n_heads (grouped multiquery attention)
...
class Transformer(nn.Module):
    ...
    def generate(
        ...
        # idx = (idx if idx.size(1) <= self.params.max_seq_len else idx[:, -self.params.max_seq_len :])
        print(f"idx after shape: {idx.shape}")

        min_prompt_len = min(len(t) for t in idx)
        max_prompt_len = max(len(t) for t in idx)
        assert max_prompt_len <= self.params.max_seq_len
        total_len = min(self.params.max_seq_len, max_new_tokens + max_prompt_len)

        prev_pos = 0
        for cur_pos in range(min_prompt_len, total_len):
            logits = self.forward(
                tokens=idx[:, prev_pos:cur_pos], start_pos=prev_pos
            )
            print(f"logits before: {logits.shape}")
            logits = logits[:, -1] # crop to just the final time step
            print(f"logits after: {logits.shape}")
