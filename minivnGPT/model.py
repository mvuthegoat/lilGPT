...
class Head(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.register_buffer('tril', torch.tril(torch.ones(config.context_length, config.context_length)))  # Save trill as the model's attribute
...
class FeedForward(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.net = nn.Sequential(
        nn.Linear(config.d_model, 4 * config.d_model), # Use the out_features as 4 * d_model as the paper uses
        nn.ReLU(),
...
@dataclass
class minivnGPTConfig:
  # Hyperparameters
  batch_size: int = 16
  context_length: int = 512
  d_model: int = 384
  n_head: int = 6
  #d_k: int = 16 # (= d_model // n_head)
  n_layer: int = 6
  dropout_rate: float = 0.0
  vocab_size: int = 32000 # 119547 for distilbert-base-multilingual-cased
...
class minivnGPT(nn.Module):
  ...
  def __init__(self, config):   # Suggested parameters: (self, n_layer, d_model, n_head, context_length)
    super().__init__()
    ...
    self.decoder_layers = nn.Sequential(*[Block(self.config) for _ in range(config.n_layer)])
  ...
  def forward(self, x, targets = None):
    ...
    position_emb = positional_encoding(x.shape[1], self.config.d_model) # x.shape[1] is the context_length of input
...
class minivnGPTForQA(nn.Module):
  def __init__(self, pre_trained_model, config):
    ...
    self.qa_outputs = nn.Linear(config.vocab_size, config.num_labels) # last linear layer will generate output of 2 dimensions
  ...
  def forward(
    ...
    # if starting and ending positions are out of range (context_length), clamp them within context_length
      ignored_index = start_logits.shape[1]  # start_logits.shape[1] is the context_length
